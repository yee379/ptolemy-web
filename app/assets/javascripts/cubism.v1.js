...
cubism_contextPrototype.graphite = function(host) {
  ...
  source.metric = function(expression) {
    ...
    var metric = context.metric(function(start, stop, step, callback) {
      ...
      if (step !== 1e4) target = "summarize(keepLastValue(" + target + "),'"
...
cubism_contextPrototype.metric = function(request, name) {
  ...
  metric.recentValue = function(max) {
    if( typeof(max) === 'undefined' )
      max = 0;
    var v = NaN
    var t = NaN
    for( var i=values.length; i>=values.length-max; i-- ) {
      v = parseFloat(metric.valueAt(i))
      // console.log(" v: " + i + " = " + v)
      if( ! isNaN(v) ) {
        t = i
        break;
    ...
    t = new Date(stop - ((values.length - t) * step));
    // console.log( 'time: ' + t )
    return v
  };
  ...
  metric.klass = function() {
    return klass
  };
  metric.height = function() {
    return height
  };
...
cubism_metricConstantPrototype.length = function() {
  return 0;
};
...
function cubism_metricOperator(name, operate) {
  ...
  cubism_metricOperatorPrototype.length = function() {
    return this.left.length();
  };

  cubism_metricOperatorPrototype.klass = function() {
    return this.left.klass();
  };
  cubism_metricOperatorPrototype.height = function() {
    return this.left.height();
  };
  
  cubism_metricOperatorPrototype.recentValue = function(max) {
    if( typeof(max) === 'undefined' )
      max = 0;
    
    // use at temp hold for lengths
    var v = 0;
    var w = 0;
    ...
    if( typeof this.right !== 'number' )
      w = this.right.length();
    var l = Math.max( v,w );
    // TODO: deal with right side
    for( var i=l; i>=l-max; i-- ) {
      v = parseFloat(this.left.valueAt(i));
      w = parseFloat(this.right.valueAt(i));
      if( ! isNaN(v) && ! isNaN(w) ) {
        break
      }
    }
...
cubism_metricPrototype.min = cubism_metricOperator("min", function(left, right) {
  return Math.min( left, right );
});

cubism_metricPrototype.max = cubism_metricOperator("max", function(left, right) {
...
cubism_contextPrototype.horizon = function() {
  ...
  function horizon(selection) {
    ...
    selection.each(function(d, i) {
      ...
      var colors_ = typeof colors === "function" ? colors.call(that, d, i) : colors;
      if( 'colours' in metric_ ) {
        colors_ = metric_.colours;
      } 
  ...
  horizon.height = function(_) {
    ...
    // return horizon;
    if (!arguments.length) return metric_.height;
    buffer.height = metric_.height = +_;
    return horizon;
  };
...
cubism_contextPrototype.tile = function() {
  var context = this,
      buffer = document.createElement("canvas"),
      width = buffer.width = 30,
      height = buffer.height = 30,
      scale = d3.scale.linear().interpolate(d3.interpolateRound),
      metric = cubism_identity,
      extent = null,
      title = cubism_identity,
      format = d3.format(".2s"),
      colors = [ '#c2c2c2', '#c4df9b', '#fdc68a', '#f7977a' ];

  function tile(selection) {

    // selection
    //     .on("mousemove.tile", function() { context.focus(Math.round(d3.mouse(this)[0])); })
    //     .on("mouseout.tile", function() { context.focus(null); });

    ref = selection.append("a")
    ref_can = ref.append('canvas')
    ...
    selection.each(function(d, i) {
      ...
      var that = this,
          ...
          extent_ = typeof extent === "function" ? extent.call(that, d, i) : extent,
          scale_ = metric_.scale ? metric_.scale : scale,
          start = -Infinity,
          step = context.step(),
          canvas = d3.select(that).select('a').select("canvas"),
          x0 = 0,
          y0 = 0,
          ready;
      ...
      ref_can
          .attr("width", metric_.width)
          .attr("height", metric_.height);
      ...
      function change(start1, stop) {
        ...
        // clear for the new data
        canvas.clearRect(x0, y0, metric_.width - x0, metric_.height -y0);

        // draw last value
        var v = metric_.recentValue(2)
        var color = scale_( v )
      ...
      // Update the chart when the context changes.
      context.on("change.tile-" + id, change);
      context.on("focus.tile-" + id, focus);

      // Display the first metric change immediately,
      // but defer subsequent updates to the canvas change.
      // Note that someone still needs to listen to the metric,
      // so that it continues to update automatically.
      metric_.on("change.tile-" + id, function(start, stop) {
        change(start, stop), focus();
        if (ready) metric_.on("change.tile-" + id, cubism_identity);
      });
    });
  }

  tile.remove = function(selection) {

    selection
        .on("mousemove.tile", null)
        .on("mouseout.tile", null);
    ...
    function remove(d) {
      d.metric.on("change.tile-" + d.id, null);
      context.on("change.tile-" + d.id, null);
      context.on("focus.tile-" + d.id, null);
    }
  };

  tile.mode = function(_) {
    if (!arguments.length) return mode;
    mode = _ + "";
    return tile;
  };

  tile.height = function(_) {
    if (!arguments.length) return height;
    buffer.height = height = +_;
    return tile;
  };

  tile.width = function(_) {
    if (!arguments.length) return width;
    buffer.width = width = +_;
    return tile;
  };
  
  tile.metric = function(_) {
    if (!arguments.length) return metric;
    metric = _;
    return tile;
  };
  ...
  tile.title = function(_) {
    if (!arguments.length) return title;
    title = _;
    return tile;
  };

  tile.format = function(_) {
    if (!arguments.length) return format;
    format = _;
    return tile;
  };

  tile.colors = function(_) {
    if (!arguments.length) return colors;
    colors = _;
    return tile;
  };
...
cubism_contextPrototype.gauge = function() {
  var context = this,
      mode = "offset",
      buffer = undefined,
      width = 30,
      ...
      size = 150,
      size_factor = 0.9,
      radius_factor = 0.97 / 2,
      radius = size * radius_factor,
      inner_radius_factor = 0.65,
      ...
      thresholds = [ 0, 0.6, 0.8, 1.0 ], // as fraction
      // colors = [ '#c2c2c2', '#c4df9b', '#fdc68a', '#f7977a' ],
      zones = [
        { from: thresholds[0]*max, to: thresholds[1]*max, color: '#c4df9b' },
        { from: thresholds[1]*max, to: thresholds[2]*max, color: '#fdc68a' },
        { from: thresholds[2]*max, to: thresholds[3]*max, color: '#f7977a' }
      ]
      ;
      // greenZones = [ { from: thresholds[0]*max, to: thresholds[1]*max }],
      // yellowZones = [ { from: thresholds[1]*max, to: thresholds[2]*max }],
      // redZones = [ { from: thresholds[2]*max, to: thresholds[3]*max }];

  function gauge(selection) {

    selection
        .on("mousemove.gauge", function() { context.focus(Math.round(d3.mouse(this)[0])); })
        .on("mouseout.gauge", function() { context.focus(null); });
    ...
    ref = selection.append("a")
		...
		buffer.append("svg:circle")
					.attr("cx", cx)
					.attr("cy", cy)
					.attr("r", size_factor * radius)
					.style("fill", "#fff")
					.style("stroke", "#e0e0e0")
					.style("stroke-width", "2px");
					
    for( var c in zones ) {
      // drawBand( zones[c].from, zones[c].to, colors[parseInt(c)+1] );
      drawBand( zones[c].from, zones[c].to, zones[c].color );
    }
      ...
      buffer.append("svg:text")
		...
		for (var major = min; major <= max; major += majorDelta)
		{
			...
			for (var minor = major + minorDelta; minor < Math.min(major + majorDelta, max); minor += minorDelta)
			{
				...
				var point2 = valueToPoint(minor, outer_radius_factor);
				
				buffer.append("svg:line")
  				.attr("x1", point1.x)
  				.attr("y1", point1.y)
  				.attr("x2", point2.x)
  				.attr("y2", point2.y)
			...
			var point2 = valueToPoint(major, outer_radius_factor);	
		...
		var pointerContainer = buffer.append("svg:g").attr("class", "pointerContainer");
		...
		pointerContainer.append("svg:circle")
							.attr("cx", cx)
							.attr("cy", cy)
							.attr("r", pointer_container_factor * radius)
							.style("fill", "#192823")
							.style("stroke", "#333")
							.style("opacity", 0.7);
    ...
    selection.each(function(d, i) {
      var that = this,
          ...
          step = context.step(),
          canvas = d3.select(that).select("canvas"),
          // link = d3.select(that).select("a"),
          max_,
          ready;

      function update(value) {

      	var pointerContainer = buffer.select(".pointerContainer");
    		var pointer = pointerContainer.selectAll("path");
    
    		pointerContainer.selectAll("text").text(format(value));
            ...
            if( isNaN(pointerValue) ) pointerValue = min - 0.02*range;
            if (isNaN(that._currentRotation)) that._currentRotation = min;
            ...
            // console.log( 'value at ' + value + ", from " + that.currentRotation + " to " + targetRotation);
      ...
      function change(start1, stop) {
        ...
        if( metric_.url )
          d3.select(that).select('a').attr('href',metric_.url);
        ...
        // console.log(metric_);
        var value = metric_.recentValue(3);
        update( value );
      ...
      // Update the chart when the context changes.
      context.on("change.gauge-" + id, change);
      context.on("focus.gauge-" + id, focus);

      // Display the first metric change immediately,
      // but defer subsequent updates to the canvas change.
      // Note that someone still needs to listen to the metric,
      // so that it continues to update automatically.
      metric_.on("change.gauge-" + id, function(start, stop) {
        change(start, stop), focus();
        if (ready) metric_.on("change.gauge-" + id, cubism_identity);
      });
    });
  ...
  valueToPoint = function(value, factor) {
		return  { x: cx - radius * factor * Math.cos(valueToRadians(value)),
	...
	function drawBand(start, end, color) {
		...
		buffer.append("svg:path")
				.style("fill", color)
				.attr("d", d3.svg.arc()
					.startAngle(valueToRadians(start))
					.endAngle(valueToRadians(end))
					.innerRadius( inner_radius_factor * radius)
					.outerRadius( outer_radius_factor * radius) )
	...
	function buildPointerPath(value) {
		...
		var head = valueToPoint(value, outer_radius_factor);
		var head1 = valueToPoint(value - delta, pointer_container_factor);
		var head2 = valueToPoint(value + delta, pointer_container_factor);

		var tailValue = value - (range * (1/(270/360)) / 2);
		var tail = valueToPoint(tailValue, 0.28);
		var tail1 = valueToPoint(tailValue - delta, pointer_container_factor);
		var tail2 = valueToPoint(tailValue + delta, pointer_container_factor);

		return [head, head1, tail2, tail, tail1, head2, head];

		function valueToPoint(value, factor){
			var point = { x: cx - radius * factor * Math.cos(valueToRadians(value)),
  					        y: cy - radius * factor * Math.sin(valueToRadians(value)) 
                  };
			point.x -= cx;
			point.y -= cy;
  ...
  gauge.remove = function(selection) {

    selection
        .on("mousemove.gauge", null)
        .on("mouseout.gauge", null);
    ...
    function remove(d) {
      d.metric.on("change.gauge-" + d.id, null);
      context.on("change.gauge-" + d.id, null);
      context.on("focus.gauge-" + d.id, null);
    }
  };



  gauge.mode = function(_) {
    if (!arguments.length) return mode;
    mode = _ + "";
    return gauge;
  };

  gauge.size = function(_) {
    if (!arguments.length) return size;
    size = +_;
    return gauge;
  };

  gauge.max = function(_) {
  ...
  gauge.scale = function(_) {
    if (!arguments.length) return scale;
    scale = _;
    return gauge;
  };

  gauge.extent = function(_) {
    if (!arguments.length) return extent;
    extent = _;
    return gauge;
  };

  gauge.title = function(_) {
    if (!arguments.length) return title;
    title = _;
    return gauge;
  };

  gauge.format = function(_) {
    if (!arguments.length) return format;
    format = _;
    return gauge;
  };

  gauge.colors = function(_) {
    if (!arguments.length) return colors;
    colors = _;
    return gauge;
  };
  gauge.url = function(_) {
    if (!arguments.length) return url;
    url = _;
    return gauge;
  };
...
cubism_contextPrototype.colourisor = function() {
  var context = this,
      buffer = document.createElement("canvas"),
      width = buffer.width = 30,
      height = buffer.height = 30,
  ...
  function colourisor(selection) {
    ...
    selection.each(function(d, i) {
      ...
      var that = this,
          ...
          // colors_ = typeof colors === "function" ? colors.call(that, d, i) : colors,
          extent_ = typeof extent === "function" ? extent.call(that, d, i) : extent,
          // scale_ = typeof scale === "function" ? scale.call(that, d, i) : scale,
          start = -Infinity,
          step = context.step(),
          ready;

      d3.selectAll(metric_.target)
        .datum({id: id, metric: metric_});
      ...
      function change(start1, stop) {
        ...
        var color_by = 'fill';
        if( metric_['style'] ) {
          color_by = metric_['style'];
        }
        d3.selectAll(metric_.target)
          .style(color_by, color)
      ...
      context.on("change.colourisor-" + id, change);
      context.on("focus.colourisor-" + id, focus);
      metric_.on("change.colourisor-" + id, function(start, stop) {
        change(start, stop), focus();
        if (ready) metric_.on("change.colourisor-" + id, cubism_identity);
      });
    });
  }

  colourisor.remove = function(selection) {

    selection
        .on("mousemove.colourisor", null)
        .on("mouseout.colourisor", null);
    ...
    function remove(d) {
      d.metric.on("change.colourisor-" + d.id, null);
      context.on("change.colourisor-" + d.id, null);
      context.on("focus.colourisor-" + d.id, null);
    }
  };

  colourisor.mode = function(_) {
    if (!arguments.length) return mode;
    mode = _ + "";
    return colourisor;
  };

  colourisor.height = function(_) {
    if (!arguments.length) return height;
    buffer.height = height = +_;
    return colourisor;
  };

  colourisor.width = function(_) {
    if (!arguments.length) return width;
    buffer.width = width = +_;
    return colourisor;
  };
  
  colourisor.metric = function(_) {
    if (!arguments.length) return metric;
    metric = _;
    return colourisor;
  };

  colourisor.scale = function(_) {
    if (!arguments.length) return scale;
    scale = _;
    return colourisor;
  };

  colourisor.extent = function(_) {
    if (!arguments.length) return extent;
    extent = _;
    return colourisor;
  };

  colourisor.title = function(_) {
    if (!arguments.length) return title;
    title = _;
    return colourisor;
  };

  colourisor.format = function(_) {
    if (!arguments.length) return format;
    format = _;
    return colourisor;
  };

  colourisor.colors = function(_) {
    if (!arguments.length) return colors;
    colors = _;
    return colourisor;
  };
  colourisor.url = function(_) {
    if (!arguments.length) return url;
    url = _;
    return colourisor;
  };

class ReplacementsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @replacement.save
        ...
        format.json { render action: 'show', status: :created, location: @replacement }
      else
  ...
  def destroy
    @replacement.destroy
    respond_to do |format|
      format.html { redirect_to replacements_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def replacement_params
      params.require(:replacement).permit(:entity__meta_id, :type, :quantity, :hardware_id, :replacement_id)
    end
end
